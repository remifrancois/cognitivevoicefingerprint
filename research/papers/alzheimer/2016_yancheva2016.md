# Yancheva2016 et al. (2016) — Full Text Extraction

**Source file:** 2016_yancheva2016.pdf
**Pages:** 5
...
of the ﬁrst efforts to handle multi-word
events and also the ﬁrst attempt to use
...
only focused on the oversimpliﬁed scenario
of single-token event detection.
• Such CNN-based approaches require a ﬁxed
...
ﬁrst work to use RNNs for event detection. This
is also one of the ﬁrst efforts to handle multi-word
event nuggets. Experimental results conﬁrm that
FBRNN is competitive compared to the state-of-
the-art on the ACE 2005 dataset and the Rich ERE
2015 event detection task.
2
Proposed Model
Let x = [w0, w1, ..., wn] be a sentence. We ﬁrst go
...
position to the event nugget candidate.
In
this work, to reduce the complexity of our
model and avoid overﬁtting, we only learn
...
ﬁxed size representation of its input. We concate-
nate the representations from the three branches
and pass it through a fully connected neural net-
work with a softmax output node that classiﬁes
each event candidate as an event of speciﬁc type
...
didate.
To avoid overﬁtting, we use dropout (Hinton
...
effective representations for this speciﬁc task.
3
Experiments
In this section, we ﬁrst empirically examine some
...
3.1
Datasets, candidate generation and
hyper-parameters
...
a 34-class classiﬁcation problem. In our ex-
...
siﬁcation problem (considering “non-event”
...
ate event nugget candidates, we ﬁrst followed
...
3.2
Exploration of different design choices
We ﬁrst design some experiments to evaluate the
...
different conﬁgurations on Rich ERE 2015.
Methods
P
R
F1
Sentence level in Ji and
-
-
59.7
Grishman (2008)
...
We ﬁrst note that the deep learning methods
(CNN and FBRNN) achieve signiﬁcantly better F1
...
Heng Ji and Ralph Grishman. 2008. Reﬁning Event
...
in Chinese Argument Extraction.
Association for
...
ﬁed Model of Phrasal and Sentential Evidence for
...
Manning.
2014.
Global Belief Recursive Neural
